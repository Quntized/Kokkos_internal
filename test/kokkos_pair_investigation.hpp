...
namespace Test {
    ...
    static void test_device_pair_detail_test(){
        ...
        Kokkos::parallel_for("test_detail_Kokkos_pair_in_view_in_mdspan_folder",1,KOKKOS_LAMBDA(const int i){
    ...
    TEST(kokkos_index_pair_like, host_execution){
        auto val = Kokkos::detail::index_pair_like<Kokkos::pair<int,double>,int>::value;
        EXPECT_EQ(val,true);
    }
